...
const postNew = (req, res, next) => { // todo nested promises -> async/await?
  ...
  models.Room
    .findById(bid.roomId)
    .then((room) => {
      if (room.stopAt < Date.now()) { // todo is it better to use validation errors from mogoose?
        return next(new Errors.Unprocessable('This auction is finished'));
      }

      if (bid.price - room.currentPrice < room.currentPrice * 0.05) {
      ...
      return bid
        .save()
        .then((savedBid) => {
          room.currentPrice = savedBid.price;
          room.currentBid = savedBid.id;
          room.bids.push(savedBid.id);
...
const getById = (req, res, next) => {
  req.requestedBid
    ...
    .then((bid) => {
      ...
      extendedLog && log.info(`Get room id=${bid.id}`);
    })
    .catch(next);
};
